...
[[ -n "$SENDER_ID" ]] || fail "TELEGRAM_SENDER_ID is required for post-restart validation"

INSPECT_OUTPUT="$(python3 "$INSPECTOR" --target rescue-local --sender "$SENDER_ID" 2>&1)" || fail "rescue inspector failed"
...
grep -Fq -- "[OK] requested sender appears in global telegram allowlist: $SENDER_ID" <<<"$INSPECT_OUTPUT" || fail "sender missing from global telegram allowlist after restart"
